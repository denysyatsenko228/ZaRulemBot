...
class HandlerAllText(Handler):
    ...
    def pressed_btn_category(self, message):
        self.bot.send_message(message.chat.id, "Каталог категорий товара",
                              reply_markup=self.keyboards.remove_menu(self))
    ...
    def send_message_order(self, product_id, quantity, message):
        # Answer to user after some actions
            ...
            if message.text == config.KEYBOARD['ORDER']:
                if self.DB.count_rows_order() > 0:
                    ...
                    self.bot.send_message(message.chat.id,
                                          ...
                                          reply_markup=self.keyboards.category_menu())
            # menu of categories
            if message.text == config.KEYBOARD['AUTO_CHEMISTRY']:
                self.pressed_btn_product(message, 'AUTO_CHEMISTRY')

            if message.text == config.KEYBOARD['AUTO_PARTS']:
                self.pressed_btn_product(message, 'AUTO_PARTS')

            if message.text == config.KEYBOARD['AUDIO_AND_ELECTRONICS']:
                self.pressed_btn_product(message, 'AUDIO_AND_ELECTRONICS')
